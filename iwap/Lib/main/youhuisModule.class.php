...
class youhuisModule extends MainBaseModule
{
	...
	public function index()
	{
		...
		$data = request_api("youhuis","index",$param);
// 		print_r($data);exit;
		...
		{		
			...
			$quan_list[$k]["url"] = wap_url("index","youhuis",$tmp_url_param);
				
			foreach($v['quan_sub'] as $kk=>$vv)
			{		
				$tmp_url_param = $param;
				$tmp_url_param['qid']=$vv['id'];
				$quan_list[$k]["quan_sub"][$kk]["url"] = wap_url("index","youhuis",$tmp_url_param);
			}		
		}
		...
		foreach($navs as $k=>$v)
		{
			...
			$navs[$k]['url'] = wap_url("index","youhuis",$tmp_url_param);
		}
		$data['navs'] = $navs;
		//end navs
