...
class Groupuser_medal implements medal {
	...
	public function get_medal()
	{
		...
		{	
			$group_info = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."topic_group where is_effect=1 and user_id = ".$user_id);		
			if($group_info>0)
//			if(1)
			{
			...
			{
				$result['status'] = 0;
				$result['info'] = "您还不是小组组长或已申请的小组还未通过审核";
				//$result['jump'] = url("index","group#create");
			}
		}
		return $result;
	}
