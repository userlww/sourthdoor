...
class Newuser_medal implements medal {
	...
	public function get_medal()
	{
		...
		{			
			if($user_info['mobile']!=""&&$user_info['province_id']!=0&&$user_info['city_id']!=0&&$user_info['byear']!=0&&$user_info['bmonth']!=0&&$user_info['bday']!=0&&get_user_avatar($user_id,"small")!=APP_ROOT."/public/avatar/noavatar_small.gif")
//			if(1)
			{
				...
				$link_data['create_time'] = NOW_TIME;
			...
			{
				$result['status'] = 0;
				$result['info'] = "您的个人资料以及头像未完善";
				//$result['jump'] = url("index","uc_account");
			}
		}
		return $result;
	}
	...
	public function check_medal()
	{
		...
		if($medal_log&&NOW_TIME>=($medal_log['create_time']+30*24*3600))
		{
				//超过三十天
