...
$lang = array(
	'name'	=>	'可配置的免运费',
	'description'	=>	'购买XX件免运费接口',
	'free_delivery_buy_count'	=>	'免运费数量'
);

$config = array(
	'free_delivery_buy_count' =>	''
);

/* 模块的基本信息 */
if (isset($read_modules) && $read_modules == true)
{
    $module['class_name']    = 'Freebynumber';
...
class Freebynumber_promote implements promote {
	public function count_buy_total($region_id,
		...
		//取出接口配置
		$promote_obj = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."promote where class_name='Freebynumber'");
		$promote_cfg = unserialize($promote_obj['config']);
		
		$buy_total = 0;
		foreach($goods_list as $k=>$v)
		{
			$buy_total+=intval($v['number']);
		}

		
		
		if($buy_total>=intval($promote_cfg['free_delivery_buy_count']))
		{
