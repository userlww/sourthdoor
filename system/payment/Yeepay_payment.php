...
class Yeepay_payment implements payment {
	...
	public function get_payment_code($payment_notice_id)
	{
        ...
        $data_merchant_id =  strim($payment_info['config']['yeepay_account']);
        ...
        $data_pay_key     = strim($payment_info['config']['yeepay_key']);
        $data_pay_account = strim($payment_info['config']['yeepay_account']);
        ...
        $code .= "<button class='ui-button paybutton' rel='blue' type='submit'>前往易宝在线支付</button>";
	...
	public function response($request)
	{
        ...
        $message_type   = strim($request['r0_Cmd']);
        $succeed        = strim($request['r1_Code']);   // 获取交易结果,1成功,-1失败
        $trxId          = strim($request['r2_TrxId']);  //易宝的交易流水号
        
        $amount         = strim($request['r3_Amt']);    // 获取订单金额
        $cur            = strim($request['r4_Cur']);    // 获取订单货币单位
        $product_id     = strim($request['r5_Pid']);    // 获取产品ID
        $orderid        = strim($request['r6_Order']);  // 获取订单ID
        $userId         = strim($request['r7_Uid']);    // 获取产品ID
        $merchant_param = strim($request['r8_MP']);     // 获取商户私有参数
        $bType          = strim($request['r9_BType']);  // 获取订单ID

        $mac            = strim($request['hmac']);      // 获取安全加密串
