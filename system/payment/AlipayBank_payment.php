...
class AlipayBank_payment implements payment {
	...
	{		
		...
		if($payment_item)
		{
			...
			$html = "<style type='text/css'>.alibank_types{ background:url(".SITE_DOMAIN.APP_ROOT."/system/payment/AlipayBank/banklogo.gif);}";
	        $html .=".bk_typeCMB{background-position:15px -450px; }";  //招行
	        $html .=".bk_typeICBCB2C{background-position:15px -410px; }";  //工行
	        $html .=".bk_typeCCB{background-position:15px -90px; }"; //建行
	        $html .=".bk_typeABC{background-position:15px -50px; }"; //农行
	        $html .=".bk_typeSPDB{background-position:15px -370px; }"; //上海浦东发展银行
	        $html .=".bk_typeSDB{background-position:15px -330px; }"; //深圳发展银行
	        $html .=".bk_typeCIB{background-position:15px -490px; }"; //兴业银行
	        $html .=".bk_typeBJBANK{background-position:15px -616px; }"; //北京银行
	        $html .=".bk_typeCEBBANK{background-position:15px -130px; }"; //光大银行
	        $html .=".bk_typeCMBC{background-position:15px -170px; }"; //民生银行
	        $html .=".bk_typeCITIC{background-position:15px -290px; }"; //中信银行
	        $html .=".bk_typeGDB{background-position:15px -250px; }"; //广东发展银行
	        $html .=".bk_typeSPABANK{background-position:15px -909px; }"; //平安银行
	        $html .=".bk_typeBOCB2C{background-position:15px -942px; }"; //中国银行
	        $html .=".bk_typeCOMM{background-position:15px -210px; }"; //交通银行
	        $html .=".bk_typeICBCBTB{background-position:15px -788px; }"; //工行企业
	        $html .=".bk_typePSBC-DEBIT{background-position:15px -530px; }"; //中国邮政储蓄银行(银联)
	        $html .="</style>";
	        ...
	        {
	            $html  .= "<label class='alibank_types bk_type".$key." ui-radiobox' rel='payment_rdo'><input type='radio' name='payment' value='".$payment_item['id']."' rel='".$key."' onclick='set_bank(\"".$key."\")' /></label>";
	        }
