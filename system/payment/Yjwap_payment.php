...
class Yjwap_payment implements payment {
	...
	public function get_payment_code($payment_notice_id)
	{
		...
		if($from=="wap")
			...
			$data_return_url = SITE_DOMAIN.APP_ROOT.'/callback/payment/yjwap_response_app.php';
		...
		* (
		...
		$pay['is_wap'] = 1;//
		$pay['pay_code'] = 'yjwap';//,支付宝;mtenpay,财付通;mcod,货到付款
		*/
		
		$pay['pay_info'] = $title_name;
		$pay['pay_action'] = $url;
		$pay['payment_name'] = '易宝一键支付wap';//$payment_info['name'];
		$pay['pay_money'] = $money;
	...
	public function response($request)
	{
		...
		//file_put_contents(APP_ROOT_PATH."system/payment/log/yjwap_response_".strftime("%Y%m%d%H%M%S",time()).".txt",print_r($request,true));
	...
	public function notify($request)
	{
		...
		//file_put_contents("./log/yjwap_notify_".strftime("%Y%m%d%H%M%S",time()).".txt",print_r($request,true));
	...
	public function get_display_code()
	{
		return "易宝一键支付wap";
		/*
		$payment_item = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."payment where class_name='Yjwap'");
		
		return "易宝一键支付wap";		
		if($payment_item)
		{
			$html = "<label class='f_l ui-radiobox' rel='common_payment'>".
					"<input type='radio' name='payment' value='".$payment_item['id']."' />&nbsp;".
					$payment_item['name'].
					"</label>";
			if($payment_item['logo']!='')
			{
				$html .= "<div class='f_l pl10'><img src='".APP_ROOT.$payment_item['logo']."' /></div>";
			}
			$html .= "<div class='f_l pl10'>".nl2br($payment_item['description'])."</div>";
			return $html;
		}
		else
		{
