...
class Phpwind_integrate implements integrate {
   ...
   private function code_string($string, $action='ENCODE')
	...
	public function add_user($user_name,$user_pwd,$email)
	{
		 ...
		 if(!$pw_userinfo)
		 {
		            $this->pwdb->query("INSERT INTO ".$config['PREFIX']."members SET username='".$user_name."', password='".md5($user_pwd)."', email='".$email."', regdate='".NOW_TIME."'", 'SILENT');
	...
	public function install($config_seralized)
	{
        ...
        else
        {
        	...
        	$user_list = $GLOBALS['db']->getAll("SELECT * FROM " .DB_PREFIX."user ORDER BY `id` ASC");
