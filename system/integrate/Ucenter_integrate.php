...
class Ucenter_integrate implements integrate {
	...
	public function install($config_seralized)
	{
		//uc数据的安装
    	 ...
    	 $app_type   = 'OTHER';
    	 $app_name   = app_conf("SHOP_TITLE");
    	 $app_url    = SITE_DOMAIN.APP_ROOT;
    	 $app_charset = 'utf-8';
    	 $app_dbcharset = strtolower((str_replace('-', '', $app_charset)));
    	 $ucapi = !empty($post_config['UCENTER_URL']) ? strim($post_config['UCENTER_URL']) : '';
    	 $ucip = !empty($post_config['UCENTER_IP']) ? strim($post_config['UCENTER_IP']) : '';
    	 $dns_error = false;
		    if(!$ucip)
		    {
	        ...
	        $ucfounderpw = strim($post_config['UCENTER_ADMIN']);
	   		$app_tagtemplates = '';
	...
	private function import_user()
	{
		if(file_exists(APP_ROOT_PATH."public/uc_config.php"))
		{
		    ...
		    $user_lists  = $GLOBALS['db']->getAll("SELECT * FROM " .DB_PREFIX."user ORDER BY `id` ASC");
		    foreach($user_lists as $data)
		    {
		        ...
		        else
		        {
			    	...
			    	logger::save(logger::FILE,"uc_install");		        	
