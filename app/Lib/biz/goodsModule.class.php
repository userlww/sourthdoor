...
class goodsModule extends BizBaseModule
{
    ...
    public function index()
    {
        ...
        /* 业务逻辑部分 */
        $conditions .= " where d.is_effect = 1 and d.is_delete = 0 and d.is_shop = 1 and d.supplier_id = ".$supplier_id; // 查询条件
        ...
        /* 系统默认 */
        $GLOBALS['tmpl']->assign("page_title", "商品项目管理");
...
//         echo $sql . $conditions . " order by id desc limit " . $limit;exit;
    ...
    public function do_save_publish()
    {
        ...
        $data['name'] = strim($_REQUEST['name']); // 商品名称
        $data['sub_name'] = strim($_REQUEST['sub_name']); // 简短名称
        $data['brief'] = strim($_REQUEST['brief']); // 简介
        $data['buy_type'] = 0; // 默认为普通商品
        $data['brand_id'] = intval($_REQUEST['brand_id']); // 品牌
        $data['shop_cate_id'] = intval($_REQUEST['shop_cate_id']); //商品分类        
        $data['description'] = btrim(no_xss($_REQUEST['description'])); //描述 
        ...
        $data['origin_price'] = floatval($_REQUEST['origin_price']); // 原价
        $data['current_price'] = floatval($_REQUEST['current_price']); // 商品价
        
        //开启自动审核
        if($allow_publish_verify){
            $data['balance_price'] = $data['current_price']*$publish_verify_balance; // 商户结算价
        }else{
            $data['balance_price'] = floatval($_REQUEST['balance_price']); // 商户结算价
        }
        
        
        $data['is_refund'] = intval($_REQUEST['is_refund']);    //是否支持退款
        $data['is_delivery'] = intval($_REQUEST['is_delivery']);    //是否支持退款
        
        
        $data['cache_free_delivery'] = base64_decode(strim($_REQUEST['cache_free_delivery']));
        $data['cache_deal_delivery'] = base64_decode(strim($_REQUEST['cache_deal_delivery']));
        $data['cache_deal_payment'] = base64_decode(strim($_REQUEST['cache_deal_payment']));
        ...
        foreach($filter as $filter_group_id=>$filter_value)
        {
            ...
            $filter_data_cache[$filter_group_id] = $filter_data;
        }
        $data['cache_deal_filter'] = serialize($filter_data_cache);
        $data['deal_goods_type'] = intval($_REQUEST['deal_goods_type']); //商品类型
        ...
        $data['is_effect'] = 1;
        $data['is_delete '] = 0;
        if ($_REQUEST['deal_attr'] && count($_REQUEST['deal_attr']) > 0) {  //多套餐
            $data['multi_attr'] = 1;
        } else {
            $data['multi_attr'] = 0;
        }
        ...
        $data['cache_location_id'] = serialize($location_id);   //支持门店缓存
        $data['cache_focus_imgs'] = serialize($focus_imgs);     //图集缓存
        ...
        foreach ($deal_attr as $goods_type_attr_id => $arr) {
            foreach ($arr as $k => $v) {
                if ($v != '') {
                    ...
                    //开启自动审核，结算金额必须根据 比例计算
                    if($allow_publish_verify){
                        $deal_attr_item['add_balance_price'] = $deal_attr_item['price']*$publish_verify_balance;
                    }else{
