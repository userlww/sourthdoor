...
class biz_storerModule extends MainBaseModule
{
     ...
     *  [item] => Array 商户下所有门店列表
        (
            [0] => Array
                (
                    [id] => 97   [int] 门店编号   
                    [name] => 香奈儿COCO小姐      [string]商品名称
                    [good_rate] => 0.66    [float] 好评率
                    [ref_avg_price] => 10.66    [float]人均消费
	...
	public function index(){
	    ...
	    $result = $GLOBALS['db']->getAll("select id,name,good_rate,ref_avg_price,avg_point,dp_count from ".DB_PREFIX."supplier_location where ".$condition." order by id desc limit ".$limit);
	    $count = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."supplier_location where ".$condition);
	    //分页
	    ...
	    if($result){
	        foreach ($result as $k=>$v){
	            ...
	            $result[$k]['ref_avg_price'] = round($v['ref_avg_price'],2);
	            $result[$k]['good_rate'] = round($v['good_rate']*100,1);
	        }
	    }
	    $root['item'] = $result?$result:array();
	    $root['page_title'] = "门店评价列表";
     ...
     * [item]=>array(  评价数据数组
         ...
         [count] => 1  int:评价数量
         [name] => 普吉岛旅游                 string:评价数据的名称 （商品/团购/门店/活动）
         [star_1] => 0       int:一星人数
         [star_2] => 0       int:二星人数
         [star_3] => 1       int:三星人数
         [star_4] => 0       int:四星人数
    ...
    public function storer_dp_list(){
        ...
        $result = wap_biz_get_dp_list($limit,"store", $data_id,$is_bad);
