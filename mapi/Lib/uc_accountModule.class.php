...
class uc_accountModule extends MainBaseModule
{
	...
	public function save()
	{
		...
		{			
			if($user_name=="")
			{
				output($root,0,"请输入您的用户名");
			}
			if($email=="")
			{
				output($root,0,"请输入您的真实邮箱");
			}
			if($user_pwd=="")
			{
				output($root,0,"请设置您的登录密码");
			}
			...
			else
			{
				...
				output($root,0,$error_msg);
			}			
			//
		}
		else
		{
			output($root,0,"资料已经更新");
		}
