...
class biz_eventrModule extends MainBaseModule
{
     ...
     *  [item] => Array 商户下所有活动列表
        (
            [0] => Array
                (
                    [id] => 97   [int] 活动编号   
                    [name] => 香奈儿COCO小姐      [string]商品名称
                    [total_count] => 10    [int]活动名额总数
                    [submit_count] => 10    [int]活动报名人数
	...
	public function index(){
	    ...
	    $result = $GLOBALS['db']->getAll("select id,name,total_count,submit_count,avg_point,dp_count from ".DB_PREFIX."event where ".$condition." order by id desc limit ".$limit);
	    $count = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."event where ".$condition);
	    //分页
     ...
     * [item]=>array(  评价数据数组
         ...
         [count] => 1  int:评价数量
         [name] => 普吉岛旅游                 string:评价数据的名称 （商品/团购/活动/活动）
         [star_1] => 0       int:一星人数
         [star_2] => 0       int:二星人数
         [star_3] => 1       int:三星人数
         [star_4] => 0       int:四星人数
     ...
     */
    public function eventr_dp_list(){
        /*初始化*/
        $root = array();
        $account_info = $GLOBALS['account_info'];
        $root['page_title'] = "活动评价详情";
        ...
        $result = wap_biz_get_dp_list($limit,"event", $data_id,$is_bad);
     ...
     */
    public function eventr_reply_dp(){
        /*初始化*/
        $root = array();
        $account_info = $GLOBALS['account_info'];
        $root['page_title'] = "活动评价回复";
