...
class biz_dealrModule extends MainBaseModule
{
     ...
     *  [item] => Array 商户下所有商品列表
        (
            [0] => Array
                (
                    [id] => 97   [int] 商品编号   
                    [sub_name] => 香奈儿COCO小姐      [string]商品名称
                    [current_price] => 109.8    [float]当前价格
	...
	public function index(){
	    ...
	    $condition = "is_effect = 1 and is_delete = 0 and supplier_id=".$account_info['supplier_id'];
	      
	    $result = $GLOBALS['db']->getAll("select id,sub_name,current_price,avg_point,dp_count from ".DB_PREFIX."deal where ".$condition." order by id desc limit ".$limit);
	    ...
	    if($result){
	        foreach ($result as $k=>$v){
	            ...
	            $result[$k]['current_price'] = round($v['current_price'],2);
	        }
	    }
	    $root['item'] = $result?$result:array();
	    $root['page_title'] = "消费评价列表";
     ...
     */
    public function dealr_dp_list(){
        /*初始化*/
        $root = array();
        $account_info = $GLOBALS['account_info'];
        $root['page_title'] = "消费评价详情";
        ...
        $result = wap_biz_get_dp_list($limit,"deal", $data_id,$is_bad);
     ...
     */
    public function dealr_reply_dp(){
        /*初始化*/
        $root = array();
        $account_info = $GLOBALS['account_info'];
        $root['page_title'] = "消费评价回复";
