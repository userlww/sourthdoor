...
class biz_youhuioModule extends MainBaseModule
{
	...
	public function index(){
	    ...
	    $sql = 'select y.* from '.DB_PREFIX.'youhui y left join '.DB_PREFIX.'youhui_location_link yll on yll.youhui_id = y.id where y.is_effect = 1 AND y.supplier_id = 35 and yll.location_id in('.implode(",", $account_info['location_ids']).') GROUP BY y.id order by y.id desc';
