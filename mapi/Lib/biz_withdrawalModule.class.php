...
class biz_withdrawalModule extends MainBaseModule
{
	...
	public function index(){
	    ...
	    $list = $GLOBALS['db']->getAll("select create_time,money,status from ".DB_PREFIX."supplier_money_submit  where supplier_id=".$supplier_id." order by status asc,create_time desc limit ".$limit);
	    foreach($list as $k=>$v){
			if($v['status']==1){
				$list[$k]['status']="已确认提现";
			}else{
