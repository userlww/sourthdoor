...
class OfcAction extends CommonAction{
	...
	public function fx_sale_month_line()
	{
		...
		$sql = "select sale_money,fx_salary,fx_extend_salary,stat_time from ".DB_PREFIX."fx_statements where stat_month = '".$stat_month."'";
		$stat_result = $GLOBALS['db']->getAll($sql);
		...
		{
			$stat_time = $stat_month."-".str_pad($i,2,"0",STR_PAD_LEFT);
			$data_row = array("value"=>0,"tip"=>$stat_time."分销平台营业额0元");
			foreach($stat_result as $row)
			{
				if($row['stat_time']==$stat_time)
				{
					if($row['sale_money']>$max_value)$max_value = $row['sale_money'];
					$data_row = array("value"=>floatval($row['sale_money']),"tip"=>$stat_time."分销平台营业额".round($row['sale_money'],2)."元");
				}
			}
			$sale_line_values[] = $data_row;
		}
		$sale_line_element = array("type"=>"line","colour"=>SALE_COLOR,"text"=>"分销平台营业额","width"=>2,"values"=>$sale_line_values);
	
	
	
		//分销佣金线条元素
		$fx_salary_line_values = array();
		for($i=1;$i<=$days;$i++)
		{
			$stat_time = $stat_month."-".str_pad($i,2,"0",STR_PAD_LEFT);
			$data_row = array("value"=>0,"tip"=>$stat_time."分销佣金0元");
			foreach($stat_result as $row)
			{
				if($row['stat_time']==$stat_time)
				{
					if($row['fx_salary']>$max_value)$max_value = $row['fx_salary'];
					$data_row = array("value"=>floatval($row['fx_salary']),"tip"=>$stat_time."分销佣金".round($row['fx_salary'],2)."元");
				}
			}
			$fx_salary_line_values[] = $data_row;
		}
		$fx_salary_line_element = array("type"=>"line","colour"=>REFUND_COLOR,"text"=>"分销佣金","width"=>2,"values"=>$fx_salary_line_values);
		...
		{
			$stat_time = $stat_month."-".str_pad($i,2,"0",STR_PAD_LEFT);
			$data_row = array("value"=>0,"tip"=>$stat_time."推广佣金0元");
			foreach($stat_result as $row)
			{
				if($row['stat_time']==$stat_time)
				{
					if($row['fx_extend_salary']>$max_value)$max_value = $row['fx_extend_salary'];
					$data_row = array("value"=>floatval($row['fx_extend_salary']),"tip"=>$stat_time."推广佣金".round($row['fx_extend_salary'],2)."元");
				}
			}
			$fx_extend_salary_line_values[] = $data_row;
		}
		$fx_extend_salary_line_element = array("type"=>"line","colour"=>VERIFY_COLOR,"text"=>"推广佣金","width"=>2,"values"=>$fx_extend_salary_line_values);
	
		$max_value = ofc_max($max_value);

		$result['y_axis'] = array("max"=>floatval($max_value));
		$result['elements'] = array($sale_line_element,$fx_salary_line_element,$fx_extend_salary_line_element);
		$result['bg_colour']	= "#ffffff";
