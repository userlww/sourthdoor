...
class TopicGroupAction extends CommonAction{
	...
	{
		$condition['is_effect']=  array('neq',2);
	...
	{
		$condition['is_effect']=  array('eq',2);
		$this->assign("default_map",$condition);
		parent::index();		
	}	
	...
	public function insert() {
		...
		$data['user_id'] = intval(M("User")->where("user_name='".strim($_REQUEST['user_name'])."'")->getField("id"));	
	...
	public function apply_pass()
	{
		...
		$info = M(MODULE_NAME)->where("id=".$id)->getField("name");
		$c_is_effect = M(MODULE_NAME)->where("id=".$id)->getField("is_effect");  //当前状态
		$n_is_effect = $c_is_effect == 2 ? 1 : 2; //需设置的状态
		M(MODULE_NAME)->where("id=".$id)->setField("is_effect",$n_is_effect);	
		save_log($info.l("APPLY_PASS_".$n_is_effect),1);
		$this->ajaxReturn($n_is_effect,l("APPLY_PASS_".$n_is_effect),1)	;			
	}
	public function update() {
		...
		$data['user_count'] = M("UserTopicGroup")->where("group_id=".$data['id'])->count();
		if($data['user_id']>0)
			$data['user_count']+=1;
		// 更新数据
