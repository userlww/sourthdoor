...
class TagGroupAction extends CommonAction
{
	...
	public function edit() {		
		...
		{
			$shop_cates[$k]['checked'] = M("TagGroupSlink")->where("category_id=".$v['id']." and tag_group_id = ".$vo['id'])->count();
		}
		$this->assign("shop_cates",$shop_cates);
		...
		{
			$event_cates[$k]['checked'] = M("TagGroupElink")->where("category_id=".$v['id']." and tag_group_id = ".$vo['id'])->count();
		}
		$this->assign("event_cates",$event_cates);
	...
	public function update()
	{
		...
		if (false !== $list) {
			...
			M("TagGroupSlink")->where("tag_group_id=".$data['id'])->delete();
			foreach($_REQUEST['scate_id'] as $cate_id)
			{
				if(intval($cate_id)>0)
				{
			...
			M("TagGroupElink")->where("tag_group_id=".$data['id'])->delete();
			foreach($_REQUEST['ecate_id'] as $cate_id)
			{
				if(intval($cate_id)>0)
				{
	...
	public function foreverdelete() {
		...
		if(!empty($id))
		{
			...
			if(false !== $model->where ( $condition )->delete ())
			{
				...
				M("TagGroupSlink")->where($link_condition)->delete();
				M("TagGroupElink")->where($link_condition)->delete();
