...
class DealCouponAction extends CommonAction{
	public function index()
	{
		...
		{
			$map['password'] = array('like','%'.strim($_REQUEST['password']).'%');
		}
...
public function update() {
	...
	{
		$content = iconv("utf-8","gbk","序列号,生效时间,过期时间" . "\n");
		$timestr = to_date(NOW_TIME);
		$content .= iconv("utf-8","gbk","12345678,".$timestr . ",".$timestr . "\n");	
	...
	public function importInsert()
	{
		...
		if($content){
			foreach($content as $k=>$v)
			{
					...
					{
						$imp_row = explode(",",$v);
						$password = strim($imp_row[0]);
						$begin_time = strim($imp_row[1])=="" ? 0 : to_timespan(strim($imp_row[1]));
						$end_time = strim($imp_row[2])=="" ? 0 : to_timespan(strim($imp_row[2]));
						$res = add_coupon($deal_id,0,0,"",$password,$begin_time,$end_time);
	...
	public function export_csv($page = 1)
	{
		...
		{
			$map[DB_PREFIX.'deal_coupon.sn'] = array('like','%'.strim($_REQUEST['sn']).'%');
		}
		if(strim($_REQUEST['user_id'])!='')
		{
			$map[DB_PREFIX.'deal_coupon.user_id'] = intval($_REQUEST['user_id']);
		}
		if(intval($_REQUEST['is_valid'])>=0)
		{
		...
		if($list)
		{
			...
			if($page == 1)
	    	$content = iconv("utf-8","gbk","编号,序列号,团购编号,购买的团购,购买数量,订单号,会员名称,会员手机号,生效时间,过期时间,使用时间" . "\n");
