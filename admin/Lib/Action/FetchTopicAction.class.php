...
class FetchTopicAction extends CommonAction{
	private function read_modules()
	{
		$directory = APP_ROOT_PATH."system/fetch_topic/";
		$read_modules = true;
	...
	public function index()
	{
		...
		$db_modules = $GLOBALS['db']->getAll("select * from ".DB_PREFIX."fetch_topic");
		...
		$this->assign("fetch_topic_list",$modules);
		$this->display();
	}
	...
	public function install()
	{
		...
		$file = $directory.$class_name."_fetch_topic.php";
		if(file_exists($file))
		{
			$module = require_once($file);
			$rs = M("FetchTopic")->where("class_name = '".$class_name."'")->count();
			if($rs > 0)
			{
	...
	public function edit() {		
		...
		$file = $directory.$vo['class_name']."_fetch_topic.php";
		if(file_exists($file))
		{
			$module = require_once($file);
		}
