...
class SupplierLocationImagesAction extends CommonAction {
	public function insert() {
		...
		$model = D($name);
		$count = $model->where(array("supplier_location_id"=>intval($_REQUEST['supplier_location_id'])))->count();
		if($count>=MAX_SP_IMAGE)
		{
			ajax_return(array("status"=>false,"info"=>"图片不能超过".MAX_SP_IMAGE."张","error"=>1));
		}
		...
		if($result['status'] == 1)
		{
			...
			ajax_return(array("status"=>true,"info"=>$file_url,"error"=>0));
		}
		else
		{
			ajax_return(array("status"=>false,"info"=>"上传失败","error"=>1));
		}
...
// 		$GLOBALS['db']->query("update ".DB_PREFIX."supplier_location_images set brief = '".$brief."' where id = ".$id);



// 		//成功提示
// 		save_log("商家图片ID:".$id . L("UPDATE_SUCCESS"), 1);
