...
class DeliveryAction extends CommonAction{
		...
		if (false !== $list) {
			...
			foreach($delivery_regions as $k=>$v)
			{
				if($v!='')
				{
					...
					$delivery_fee_item['supplier_id'] = 0;
	...
	public function edit() {		
		...
		$this->assign("weight_list",$weight_list);
		//开始输出配送地区列表
		$regions_list = M("DeliveryFee")->where("delivery_id=".$id." and supplier_id=0")->findAll();
	...
	public function update() {
		...
		if (false !== $list) {
			M("DeliveryFee")->where("delivery_id=".$data['id']." and supplier_id=0")->delete();
			//开始处理配送地区
			$delivery_regions = $_REQUEST['region_support_region'];
			foreach($delivery_regions as $k=>$v)
			{
