...
class TopicAction extends CommonAction{
	public function index()
	{
		...
		$reminder['topic_count_time'] = NOW_TIME;
		...
		if(strim($_REQUEST['type'])=='fav')
		{
			unset($map['type']);
			$map['fav_id'] = array("neq",0);
		}
		if(strim($_REQUEST['type'])=='relay')
		{
			unset($map['type']);
				...
				if ($list!==false) {
					...
					foreach($rel_data as $topic)
					{
						...
						if(intval(app_conf('USER_DELETE_MONEY'))<0 || intval(app_conf('USER_DELETE_POINT'))<0 || intval(app_conf('USER_DELETE_SCORE'))<0)
						{
							...
							modify_account(array("money"=>intval(app_conf('USER_DELETE_MONEY')),"score"=>intval(app_conf('USER_DELETE_SCORE')),"point"=>intval(app_conf('USER_DELETE_POINT'))),$topic['user_id'],"主题被删除");	
						}
						if($topic['fav_id']>0)
						{
