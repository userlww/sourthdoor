...
function load_coupon_time_type()
{
	$(".coupon_time_type").hide();
	var coupon_time_type = $("select[name='coupon_time_type']").val();
	$(".coupon_time_type_"+coupon_time_type).show();
...
{
	var shop_cate_id = $("select[name='shop_cate_id']").val();
	var id = $("input[name='id']").val();
	var buy_type = $("select[name='buy_type']").val();
	if(shop_cate_id>0&&buy_type==0)
	{
		var query = new Object();
		query.ajax = 1;
		...
		$.ajax({ 
			url: ROOT+"?"+VAR_MODULE+"="+MODULE_NAME+"&"+VAR_ACTION+"=filter_html", 
			data: query,
			success: function(obj){
				$("#filter").html(obj);
			}
