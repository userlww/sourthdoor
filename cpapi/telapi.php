...
define("FILE_PATH","/cpapi"); //文件目录
require_once '../system/system_init.php';

$action = strtolower(strim($_REQUEST['action'])); 
if($action=='query')
{
	//验证
	$callerid = strim($_REQUEST['callerid']);  //来电电话
	$sn = strim($_REQUEST['num']);  //团购券序列号
	$query_id = strim($_REQUEST['serialno']);  //第三方队列ID
